//! The crosshair, and the one line the game ever says.
//!
...
//! on request anyway.
//!
//! What is left here is words, and words are for the grown-up holding the machine: where
//! the ticket went, and nothing else. What the player is told is drawn instead, on
//! [`crate::hotbar`]'s pad and on [`crate::forge`]'s rig, neither of which has a letter on
//! it. That is why this whole overlay goes away while the rig is up and the pad does not.
...
/// player — the pad and the rig included, since they are laid out in the same pixels.
const DESIGN_ROWS: f32 = 800.0;

/// A top-level piece of the HUD. Marked so the crafting rig can put the words away in one
/// query while it is up.
#[derive(Component)]
pub struct HudRoot;
...
/// a pad nobody can read from a sofa. Scaling the UI rather than shrinking the window is
