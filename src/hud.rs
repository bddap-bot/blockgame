//! Everything drawn flat over the world — which, now, is a crosshair and the occasional
//! notice that fades.
//!
//! There used to be a hotbar down here: a strip of named boxes with a count in each, and a
//! line above it spelling out what the thing in your hand cost. Both are gone. What you
//! are carrying is [`crate::belt`] — your own body with your kit hanging off it, reached
//! by a two-press d-pad code — and what things are made of is [`crate::forge`]. Neither
//! writes anything down, which is the point: the players are children on Steam Decks, and
//! a surface a child cannot read is a surface that does not work.
//!
//! What is left here is text, still, and is meant to be: a notice is the game telling an
//! adult where a file went. It says its piece and stops. Nothing sits permanently at the
//! top of the screen — a status plate lived up there once carrying a 64-character join
//! ticket, which is the first thing you see across a living room and the last thing
//! anybody reads.
...
pub fn setup(mut commands: Commands) {
    ...
    commands.spawn((
        ...
        // Above the pause menu, which is what the player is looking at when this appears.
        GlobalZIndex(20),
    ));
}
...
/// Scaling the UI rather than shrinking the window is what lets the world keep the pixels:
/// the 3D camera renders at the screen's real resolution either way.
...
///
/// Assigning through the `Mut` marks the text changed whatever it says, and changed text is
/// re-shaped and re-laid-out.
