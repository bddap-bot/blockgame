...
/// The item a whole [`Kind`] is drawn as — the first symbol of the code, and a picture of
/// what is behind that key.
///
/// A real item rather than a sixth set of shapes, so pressing the hammer key and finding
/// hammers behind it is the same picture twice and not two drawings that could drift.
pub fn stands_for(kind: Kind) -> Item {
    match kind {
        Kind::Block => Item::Stone,
        Kind::Part => Item::Nail,
        Kind::Tool => Item::Hammer,
        Kind::Wearable => Item::Parachute,
        Kind::Vehicle => Item::Car,
    }
}
...
mod tests {
    ...
    /// The five kind keys are the first symbol of the code, and they are all drawn in the
    /// same grey — so unlike every other pair in the game they have **only** their shape.
    ///
    /// Measured as coverage rather than as `!=`, because a nail and a hammer were once two
    /// different part lists that painted the same tee, and an equality check said they
    /// were fine. A silhouette is sampled on a grid and the two have to differ over a
    /// tenth of it.
    #[test]
    fn no_two_kind_keys_are_the_same_silhouette() {
        let ink = |item: Item| {
            let mut on = [false; 32 * 32];
            for part in of(item) {
                for (cell, lit) in on.iter_mut().enumerate() {
                    let (x, y) = ((cell % 32) as f32 / 32.0, (cell / 32) as f32 / 32.0);
                    *lit |=
        ...
        for a in Kind::ALL {
            for b in Kind::ALL {
                ...
                let (ia, ib) = (ink(stands_for(*a)), ink(stands_for(*b)));
                let apart = ia.iter().zip(ib).filter(|(x, y)| *x != y).count();
                assert!(
                    apart * 10 > ia.len(),
                    "the {a:?} and {b:?} keys are the same picture in the same grey"
                );
            }
        }
    }
    ...
    /// Every key on the first row of the code is a picture of something really behind it.
    #[test]
    fn a_kind_key_shows_something_of_that_kind() {
        for kind in Kind::ALL {
            let item = stands_for(*kind);
            assert_eq!(item.class().kind(), *kind, "{kind:?} key shows {item:?}");
        }
    }
